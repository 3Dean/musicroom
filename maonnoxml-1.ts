...
staticModelUrls.forEach(url => {
  loader.load(url, (gltf: any) => {
    ...
    if (url === '/models/couch_left.glb') {
      couchLeft = modelScene;
    } else if (url === '/models/couch_right.glb') {
      couchRight = modelScene;
    }
...
function addCouchInteraction(couchLeft: THREE.Object3D, couchRight: THREE.Object3D) {
  ...
  const promptGeometry = new THREE.PlaneGeometry(0.5, 0.25);
  ...
  promptMesh.position.set(0, 1.5, -1.5); // Position in front of the camera
  camera.add(promptMesh);

  // Load a texture for the prompt
  const textureLoader = new THREE.TextureLoader();
  textureLoader.load('/note.svg', (texture) => {
    promptMaterial.map = texture;
